...
public class FeatureModelAnalysis2 implements LongRunningMethod<HashMap<Object, Object>> {
	...
	private final HashMap<String, AnalysisResult<?>> map = new HashMap<>();

	private final IFeatureModel fm;

	private IMonitor monitor = new NullMonitor();

	public FeatureModelAnalysis2(IFeatureModel featureModel) {
		this.fm = featureModel;
		this.cnf = CNFCreator.createNodes(featureModel);
	}
...
//				}
	...
	private void checkFeatureFalseOptional(final Iterable<IFeature> features, final CNF cnf) {
		...
		for (IFeature feature : features) {
			...
			if (parent != null && (!feature.getStructure().isMandatorySet() || !parent.getStructure().isAnd())) {
				possibleFOFeatures.add(new LiteralSet(-cnf.getVariable(parent.getName()), cnf.getVariable(feature.getName())));
			}
		}
		final List<LiteralSet> solution3 = LongRunningWrapper.runMethod(new RedundancyAnalysis(cnf, possibleFOFeatures), monitor.subTask(0));
		monitor.checkCancel();
		falseOptionalFeatures.clear();
		for (LiteralSet pair : solution3) {
			monitor.checkCancel();
			final IFeature feature = fm.getFeature(cnf.getName(pair.getLiterals()[1]));
...
//			}
	...
	private List<IFeature> checkFeatureFalseOptional2(final ISatSolver2 solver, List<IFeature> foList) {
		...
		final List<int[]> solution3 = LongRunningWrapper.runMethod(new RedundancyAnalysis(solver, possibleFOFeatures));
		for (int[] pair : solution3) {
			result.add(fm.getFeature(si.getName(pair[1])));
		}
		return result;
	}
